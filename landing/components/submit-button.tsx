 ...
 *
 * It asks for `bg-accent` and nothing about the surface under it. `.brand-surface` in
 * `globals.css` redefines `--accent` for the navy band, so the same string draws a deep
 * blue on paper and a sky blue on navy without this component knowing which it is on.
 *
...
export function SubmitButton({
  ...
  return (
    <button
      ...
      className={`${className === undefined ? "" : `${className} `}min-h-12 rounded-control bg-accent px-5 text-base font-semibold text-accent-foreground outline-none transition-opacity duration-200 hover:opacity-90 focus-visible:outline-2 focus-visible:outline-ring focus-visible:outline-offset-2 disabled:opacity-70`}
    >
