...
/**
 * What the fixture Tender is stamped with, and what every Quote here is converted into.
 *
 * A constant rather than the string repeated, because it is one fact — the Reporting
 * Currency the org held at the moment `aTender` opened this one (ADR-0036) — and the org's
 * setting is free to move underneath it, which one test below makes it do.
 */
const tenderCurrency = "THB";
...
async function createOrg(name: string): Promise<string> {
  // On the uncapped tier, because nothing in this file is about the plan and its fixtures
  // hold several open Tenders at once: a new org defaults to `free`, which allows one
  // (ADR-0040). The seeded row is *referenced* and never written — `paid` is read by
  // every suite running beside this one.
...
/**
 * A stubbed Frankfurter, remembering every currency it will have cached.
 *
 * Three rows, not one: the boundary fetches the day's euro table rather than a pair, and
 * `freezeRate` keeps every leg it was handed so one Quote in Yuan warms the fallback for
 * the rest. `fx_rates` has no org and is shared by every suite, so what this suite caused
 * to be written is what it has to take back out — the Tender's own currency and the euro
 * included.
 */
function rates(rate: number, asOf?: string, currency = "CNY") {
  for (const touched of [currency, tenderCurrency, "EUR"]) {
    currenciesTouched.add(touched);
  }

  return respondingRates(rate, asOf, currency, tenderCurrency);
}
...
describe("the frozen rate", () => {
  it("stores mid, applied and as-of, with the org's buffer applied once", async () => {
    ...
    expect(quote.unitPriceReporting).toBeCloseTo(100 * 4.9138 * 1.02, 4);
  });
  ...
  it("asks for the day the Quote was given, and stores the day ECB answered with", async () => {
    ...
    // No `base`, which is the euro — one table, every pair, and a row for each leg cached
    // on the way past. Asking `base=CNY&symbols=THB` would answer one org's question and
    // leave the fallback holding one row instead of thirty.
    expect(boundary.asked[0]).not.toContain("base=");
  ...
  it("does not convert a Quote already in the Tender's currency, or ask anybody", async () => {
    const store = await signedInAs(assignee.email);
    ...
    expect(quote.fxRateAsOf).toBe("2026-08-18");
    expect(quote.unitPriceReporting).toBeCloseTo(125.5, 4);
  });
  ...
  it("keeps what it fetched, so the next Quote has something to fall back to", async () => {
    ...
    const { data } = await service
      .from("fx_rates")
      .select("currency, rate_per_eur")
      .in("currency", ["CNY", tenderCurrency])
      .eq("as_of", "2026-08-17");

    // Both legs, from the one fetch. The pair is their quotient and is stored nowhere —
    // which is what lets the same two rows answer a Tender reporting in anything else.
    const legs = new Map(
      (data ?? []).map((row) => [row.currency, Number(row.rate_per_eur)]),
    );

    expect(legs.get(tenderCurrency)! / legs.get("CNY")!).toBeCloseTo(4.9138, 8);
  });
  ...
  it("converts into the Tender's stamped currency, not the org's setting today", async () => {
    // ADR-0036's whole sentence. The Reporting Currency is the organisation's answer, but
    // a Tender keeps the one it opened with: `unit_price_reporting` is generated over a
    // Frozen Rate, so re-aiming a live Tender at a new currency would leave every figure
    // at the digits it always had under a label those digits were never computed in.
    //
    // The stub carries no SGD leg, so a freeze that had read `getOrgSettings` instead of
    // the Tender would find no pair and refuse — which is what makes this an assertion
    // about where the currency came from rather than about arithmetic.
    const store = await signedInAs(assignee.email);

    await service
      .from("orgs")
      .update({ reporting_currency: "SGD" })
      .eq("id", orgId);
    ...
    try {
      ...
      await service
        .from("orgs")
        .update({ reporting_currency: tenderCurrency })
        .eq("id", orgId);
    }
  });
...
describe("correcting a Quote", () => {
  ...
  it("recomputes the converted figure from the corrected price", async () => {
    const store = await signedInAs(assignee.email);
    ...
    // Generated by the database from `unit_price * fx_rate_applied`, never by hand.
    expect(quote.unitPriceReporting).toBeCloseTo(200 * quote.fxRateApplied, 6);
  });
...
describe("the frozen rate under a correction", () => {
  ...
  it("does not convert a same-currency Quote whose date was corrected, or ask anybody", async () => {
    const store = await signedInAs(assignee.email);
